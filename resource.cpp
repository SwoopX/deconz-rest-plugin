...
const char *RConfigAdaptiveSensitivity = "config/adaptive_sensitivity";
...
const char *RConfigInterferenceIdentification = "config/interference_identification";
...
void initResourceDescriptors()
{
    ...
    rItemDescriptors.emplace_back(ResourceItemDescriptor(DataTypeBool, QVariant::Bool, RConfigAdaptiveSensitivity));
    ...
    rItemDescriptors.emplace_back(ResourceItemDescriptor(DataTypeBool, QVariant::Bool, RConfigInterferenceIdentification));
...
void Resource::cleanupStateChanges()
{
    while (!m_stateChanges.empty())
    {
        ...
        if (i != m_stateChanges.end())
        {
            ...
            {
                DBG_Printf(DBG_INFO, "SC state change finished: %s, %s\n", qPrintable(item(RAttrUniqueId)->toString()), qPrintable(item(RAttrUniqueId)->descriptor().suffix));
            }
